# Python batch recording example

This example mirrors [`step_by_step`](../step_by_step) but, instead of stepping
the simulation tick by tick and pulling state back on every tick, it runs the
whole horizon server-side in a single `RunAndRecord` streaming call and decodes
the columnar recording it returns.
...
python clients/python/examples/batch/main.py > clients/python/examples/batch/output.txt
```
...
```sh
cd clients/python/examples/batch && gnuplot plot_anim.gnuplot
```

This creates `clients/python/examples/batch/output.gif`.
